...
then
    case $1 in # checks the value of the first argument
        ...
        "java") # for java programs...
            cd Source/java # move to the java folder so that the executable is created there
            javac $2.java # compile the program with the name of the second argument
            echo "$2.java compiled";;
        *) echo "Error: invalid language argument" # if the first argument is neither cpp or java, let the user know they made an error
            exit 1;; # failure
    esac
